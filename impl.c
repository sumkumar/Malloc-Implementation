...
void OneBin_init(int size){
    int i, length = 256 / size, steplength;
    void *Bin_add = (void *)malloc(64* sizeof(int));
    steplength = ((Bin_add+1)-Bin_add)/length;
    OneBin_arr.head = NULL;
    for(i=0;i<length;i++){
        OneBin_node *new_node = (OneBin_node *) malloc(sizeof(OneBin_node));
        new_node->add = Bin_add+(i*steplength);
        new_node->TAG = 0;
        new_node->next = OneBin_arr.head;
        if(OneBin_arr.head != NULL){
            OneBin_arr.head->prev = new_node;
        }
        OneBin_arr.head = new_node;
    }
}

void* OneBin_alloc(){
    OneBin_node *tmp = OneBin_arr.head;
    while(tmp != NULL){
        if(tmp->TAG == 0){
            tmp->TAG = 1;
            return tmp->add;
...
void OneBin_int_frag(){
    printf("Internal fragmentation:0\n");
}